...
func New() *Prometheus {
    ...
    prom.MustRegister(p.requestsTotal)
    prom.MustRegister(p.successfulRequestsTotal)
    prom.MustRegister(p.failedRequestsTotal)
    prom.MustRegister(p.successfulRequestDuration)
    prom.MustRegister(p.eventSendsTotal)
    prom.MustRegister(p.kafkaRequestsTotal)
    prom.MustRegister(p.kafkaErrorsTotal)
    prom.MustRegister(p.kafkaRequestsDuration)
...
func (p *Prometheus) IncEventSendsCount(eventType string, appID string, appInstance string, appEnv string, project string, ack bool) {
